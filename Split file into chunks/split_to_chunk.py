...
def file_split(file_path: str, chunk_size: int):
    ...
    def save_chunks_to_directory(chunks: list, output_directory: str, file_name: str):
        try:
            os.makedirs(output_directory, exist_ok=True)  # Tạo thư mục nếu chưa tồn tại
            for i, chunk in enumerate(chunks):
                output_file = os.path.join(output_directory, f"{file_name}_chunk_{i}.bin")
    ...
    file_name = os.path.splitext(os.path.basename(file_path))[0] #Take input file's name

    chunks = list(split_file_to_chunks(file_path, chunk_size))
    save_chunks_to_directory(chunks, "output chunk file", file_name)
